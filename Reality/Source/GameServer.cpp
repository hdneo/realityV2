...
void GameServer::Handle_Incoming()
{
			...
			(len > 0 && len <= RECV_BUFFER_SIZE))
	{
		...
		if (i != Clients.end())
		{
			...
			{
				/* Caller Client Object answers to requesting IP */
		...
		else
		{
			...
			lastID++;
			Clients[IP.str()] = new GameClient(inc_addr, &Socket,lastID);
			DEBUG_LOG("Client connected [%s], now have [%d] clients",
					IP.str().c_str(), Clients_Connected());

			/* New created Client object answers to requesting IP */
			Clients[IP.str()]->HandlePacket(Buffer, len);

			/* Broadcast new client spawn to others */
			//
		}
...
/* Needs to receive the right packet to broadcast to other players 
Manager for this, maybe?

*/
void GameServer::BroadcastOthers( const ByteBuffer &message, uint32 id )
{
	for (GClientList::iterator i = Clients.begin();i != Clients.end();++i)
	{
